      ...
      (error) => {
        ...
        if (error.code === 'failed-precondition' || error.code === 'permission-denied') {
          ...
          const simpleQuery = isAdmin
            ...
            : query(collection(db, 'books'), where('userId', '==', auth.currentUser.uid));
          ...
          onSnapshot(simpleQuery, (snapshot) => {
            ...
            booksData.sort((a, b) => new Date(b.uploadedAt) - new Date(a.uploadedAt));
            console.log('Books loaded (without index):', booksData);
            setBooks(booksData);
            setLoading(false);
          });
        } else {
